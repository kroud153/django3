...
def signup(request):
    if request.method == 'GET':
        ...
        if f.is_valid():
            ...
            #회원생성
                '''
                    회원생성을 위해 f.save()를 사용할 경우, 비밀번호가 암호화되지 않은 상태로 데이터베이스에
                    저장되기 때문에 비밀번호 원본이 저장됨 따라서 회원생성에 사용할 수 있는 별도의 함수를 사용해야 함.
                 
                 User : auth 어플리케이션에 구현된 모델클래스
                 User.objects.create_user : 비밀번호를 암호화한 상태로 회원을 생성하는 함수
                '''
                new_user = User.objects.create_user(username = f.cleaned_data['username'], 
                                                    email= f.cleaned_data['email'], password= f.cleaned_data['password'])
                print('새 유저 :', new_user)
...
def signin(request):
    if request.method == 'GET':
        ...
        #따라서 오류가 발생했을 때 입력을 전달하기 위한 폼객체생성
        f = SigninForm(request.POST)
        #POST변수에서 직접 데이터 꺼내기(아이디, 비밀번호)
        id = request.POST.get('username')
        pw = request.POST.get('password')
        #아이디랑 비밀번호가 일치하는 User 모델클래스 객체 추출
        #authenticate(username, password) : 비밀번호를 암호화하고 아이디와 암호화된 비밀번호가 일치하는
        #User객체 추출 단, 아이디나 비밀번호가 틀린경우, None값을 반환함.
