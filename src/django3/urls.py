...
urlpatterns = [
    ...
    #include함수이 namespace매개변수 : app_name변수를 무시하고 하위 그룹의 이름을 지정할 수 있음
    path('auth/', include('social_django.urls', namespace = 'social')),
    path('blog/',include('blog.urls'))
]

#사용자가 이미지, 첨부파일을 요청했을때,
#웹서버 컴퓨터에 저장된 파일을 클라이언트에게 전달하도록 설정
#웹서버의 로컬하드디스크와 URL주소를 연동

#setting.py 변수값을 사용할 수 있도록 임포트
from django.conf import settings
#MEDIA_URL 과 MEDEA_ROOT를 연결하기 위한 함수 임포트
from django.conf.urls.static import static
#웹클라이언트의 파일요청 URL 주소와 실제 파일 경로를 연동함
