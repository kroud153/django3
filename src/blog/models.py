...
class Post(models.Model):
    #Category 모델클래스와 1:n관게로 연결
    #연결된 Category 객체가 삭제되려고 할떄, Post객체가 연결되어있으면 삭제가 안되도록 설정
    category = models.ForeignKey(Category,on_delete = models.PROTECT)
    headline = models.CharField('제목',max_length = 300)
    #User 모델클래스와 1:n 관게로 연결.
    #User 객체가 삭제되면 연결된 Post 객체도 같이 삭제되도록 설정
    author = models.ForeignKey(User, on_delete=models.CASCADE)
    #TextField : 글자수 제한이없는 문자열저장공간
    #default 외에 XXXField 에서 사용할수 있는 매개변수
    #null(기본값 False) : True 값을 저장하면, 데이터베이스에 객체 저장시, 해당 변수값이 비어있어도 저장되도록 허용
    #blank(기본값 False) :  True 값을 저장하면, 폼객체를 통한 사용자 입력공간(<input>)제공시, 해당 변수의 입력공간을 빈칸으로 허용
    ...
    class Meta:
        #최신글 순으로 정렬(pub_data 값을 네림차순 정렬) 
        ordering = ['-pub_date']
#글에 포함된 이미지 정보
#Post - 외래키, 이미지 저장변수
...
class PostFile(models.Model):
    post = models.ForeignKey(Post,on_delete=models.CASCADE)
    #FileField() : 모든 파일을 저장할 수 있는 저장공간 ex) 한글문서, 실행파일, 이미지파일 등)
