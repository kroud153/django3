...
#클래스 기반의 뷰를 만들떄, 제네릭뷰를 상속받아서 장고에서 제공하는 뷰 기능을 사용
...
#LoginRequiredMixin : 사용자가 비로그인 상태로 이뷰를 요청한 경우, 로그인페이지로 이동해주는 클래스
#뷰 함수 - 데코레이터, 뷰클래스 -> XXXMixin클래스 상속
class Posting(LoginRequiredMixin, FormView):
    template_name = 'blog/posting.html'
    #해당 뷰클래스가 사용할 폼클래스 지정
    form_class = PostForm
    context_object_name = 'f'
    #is_valie()함수가 True를 반환한 뒤의 처리를 form_valid()에서 함
    #사용자가 준 추가데이터(이미지,파일정보)를 처리하기 위해서 해당 함수를 오버라이딩
    #매개변수 form : form_class 변수에 지정함
    def form_valid(self, form):
        #PostForm 객체를 저장하기 전에 글쓴이 정보를 저장해야함.
        #데이터베이스에 바로저장하지 않고 연동된 모델클래스로 반환
        #p : 사용자 입력으로 제목,카테고리, 글내용이 채워진 Post객체(아직 데이터베이스에 저장되지 않음)
        p = form.save(commit = False)
        #request.user : 요청한 클라이언트의 로그인정보( User 모델 클래스 객체 )
        #이 요청을 한 클라이언트의 유저정보를 Post객체의 저장
        p.author = self.request.user
        p.save() #데이터베이스에 저장
        
        #사용자가 넘겨준 이미지, 파일마다 객체 생성
        #사용자가 넘겨준 이미지, 파일 마다 PostImage 객체 생성
