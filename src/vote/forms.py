'''
<form> : HITML 의 <form> 태그로 사용자가 웹서버에게 전달할 값들을 작성할 수 있는 공간을 생성해줌
<form> 태그 안에 <input> 태그를 작성해서 어떤 입력을 할수 있는지 지정할 수 있음
form테그의 속성으로 action과 method를 지정해야함.
action : 사용자의 입력을 웹서버의 어느 url로 보낼것인지 지정
method : GET 또는 POST 방식을 지정

장고의  form : HTML의 <form> 태그에 들어가는 <input> 태그들을 관린할 수 있는 클래스/기능
...
class QuestionForm(ModelForm):
    #Meta 클래스 : 연동하고자 하는 모델클래스에 대한 정보를 정의하는 클래스 (대문자주의)    
    class Meta:
        #모델클래스가Question모델클래스와 연동
        model = Question 
        #Question 모델클래스의 변수중 title 변수만 작성가능
        fields = ['title']
