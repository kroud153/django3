...
#질문 (Question)
#질문 제목(CharField(100 글자제한), 생성일(DataField)
#title, pub_date
class Question(models.Model):
    title = models.CharField('설문조사제목스',max_length = 100)
...
#q(Foreignkey(질문모델클래스와 연결))
class Choice(models.Model):
    name = models.CharField('답변 항목',max_length=50)
    #default : 객체 생성시 기본값 설정하는 매개변수
    votes = models.IntegerField('투표수',default=0)
    #Choice 모덿클래스가 Question 모델클래스와 1:n관계를 가짐
    ...
    #Choice 모델클래스와 q변수는 연결된 Question객체와 동일함.
    # -> Choice 객체.q.title(또는 pub_date, id)변수를 접근할 수 있음.
    #on_delete : 연결된 모델클래스의 객체가 삭제될 때 어떻게 처리 할지 지정하는 변수
    #on_delete = model.PROTECT : 연결된 모델클래스의 객체가 삭제되지 않도록 막아주는 기능
    #models.CASCADE : 연결된 모델클래스의 객체가 삭제되면 같이삭제 
    #ex) 글을 삭제하면 글안에 댓글이 사라짐.
    #models.SET_NULL : 연결된 객체가 삭제되면 아무것도 연겨하지 않은 상태 유지
    #models.SET(연결할 객첵): 연결된 객체가 삭제되면 매개변수로 넣은 객체와 연결
