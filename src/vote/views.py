...
#질문 리스트
def index(request):
    #DB 에 저장된 Question 객체 추출
    objs = Question.objects.all()
    #index.html 파일에 Question 객체리스트 전달 및 클라이언트에게 html 파일 전송
...
def detail(request, q_id):
    ...
    if request.method == "GET":
        ...
        #만약 객체가 없는 경우, 클라이언트의 잘못된 접근으로 판단해 뷰함수를 종료하고 404번 에러코드를 전달하는 함수
        #Question 객체 중 id변수 값이 q_id 와 같은 객체 추출 없으면 404번 에러 코드 전달
        obj = get_object_or_404(Question, id=q_id)
        '''
        모델 클래스A의 겍체, 모델클래스 B_set : A모델클래스와 B모델클래스가 1:n 관계(외래키 연결)
        인 경우, 해당 A객체와 연결된 B객체들을 대상으로 get(),all(),exclude(),filter() 함수들을
        사용할 수 있음
        Question 모델클래스와 Choice 모델클래스는 1:n 관계이므로, Question 객체들은
        자신과 연결된 Choice 객체들을 대상으로  get, all, exclude, filter를 사용 할 수 있음
        '''
    ...
    #POST요청에 대한 처리(투표 반영)
        ...
        request.GET  : 사용자가 GET방식으로 요청할때 넘어온 데이터가
          저장된 변수(사전형)
        request.POST : 사용자가 POST방식으로 요청할때 넘어온 데이터가 저장된
          변수(사전형) 
        '''
        ...
        #c.q.id : Choice 객체가 연갈한 Question 객체의 id변수값
        url = '/vote/result/%d' % c.q.id
        #HttpResponseRedirect(URL 주소)
        #웹 클라이언트에게 또다른 인터넷주소를 넘겨줌
...
def qregister(request):
    ...
    #POST방식 -> 사용자 입력기반의 Question객체 생성
        ...
        if form.is_valid():
            '''
               폼객체. is_valid() : 사용자 입력이 유효한경우 True 반환, 추가적으로 cLeaned_data 변수를 사용해
               데이터를 추출할 수 있음 만약에 유효한 값이 아닌경우 False 반환, 사용자의 입력을 다시받을 수 있도록
            HTML 코드를 전달
            
            모델폼객체는 연동된 모델객체를 생성할 수 있음
            모델폼객체.save() : 반환값 생성된 모델객체, 내부적으로 데이터베이스에 새로운 객체가 저장됨
            현재 QuestionForm 객체로 title 변수만 값이 작성되있어 데이터베이스에 바로 저장을 할 수 없음(pub_data변수 값없음)
...
def qupdate(request, q_id):
    ...
    if request.method == "GET":
        #instance 매개변수 : 데이터베이스에 저장된 객체를 연동할 수 있는 변수, 연동한 모델폼객체는 HTML코드로 변환시
        #기본값이 객체 변수 값으로 채워져있음
...
def cregister(request):
    if request.method == 'GET':
        ...
        c = f.save()
        #return HttpResponseRedirect(reverse('vote:index'))
        #삭제한 choice 객체가 연결된 Question객체의 detail로 이동
        return HttpResponseRedirect(
            reverse('vote:detail',args = (c.q.id,)))
#Choice 객체 수정
#공통 : 수정할 대상을 추출
#GET - Choice 객체 기반으로 ChoiceForm 객체 생성 후 HTML전달
